...
void main (void)
{
  ...
  float time = u_time * 1.;
  float radius = 0.5;
  vec2 circle = vec2(sin(time)*radius, cos(time)*radius);
  vec2 center = vec2(0.5,0.0);
  mat2 mat = getRotationMatrix(time);
  vec2 pt = mat * v_position.xy;
  float inRect = 1.0-rect(pt, vec2(0.05), center);
  float inCircle = smoothstep(0.1, 0.105, length(v_position.xy));
  
  vec3 color = vec3(0.0,0.0,0.0) + inRect;
...
function onWindowResize( event ) {
  ...
  uniforms.u_resolution.value.x = window.innerWidth*0.65;
  uniforms.u_resolution.value.y = window.innerHeight*0.65;
}
